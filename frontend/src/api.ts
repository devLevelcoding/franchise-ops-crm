const API_BASE = import.meta.env.VITE_API_BASE ?? 'http://localhost:3306'
...
export const runAction = <T>(resource: string, id: number, action: string, body: Record<string, unknown> = {}): Promise<T> =>
  req(`/${resource}/${id}/${action}`, { method: 'POST', body: JSON.stringify(body) })
