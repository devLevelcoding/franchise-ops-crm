...
import type { ColumnDef, FieldDef, Shop } from '../types'
...
export default function ResourceView<T extends { id: number }>({
  resource, title, columns, formFields, shops, quickAction,
}: Props<T>) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      for (const f of formFields) {
        ...
        body[f.key] = f.type === 'number' || f.type === 'shop-select' ? Number(raw) : raw
      }
      if (editingId !== null) {
